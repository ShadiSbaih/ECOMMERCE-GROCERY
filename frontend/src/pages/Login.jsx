      ...
      <div className="grid min-h-[calc(100vh-80px)] w-full overflow-hidden bg-[#fbfaf5] lg:grid-cols-[0.92fr_1.08fr]">
        <aside className="relative flex min-h-[390px] flex-col justify-between overflow-hidden bg-[#1d4b35] p-7 text-white sm:p-10 lg:min-h-full">
          <div className="relative z-10 mx-auto mt-8 flex max-w-md flex-col items-center text-center sm:mt-16 lg:mt-24 lg:max-w-lg">
            ...
            <h1 className="text-4xl font-extrabold leading-[1.15] tracking-tight drop-shadow-lg sm:text-5xl lg:text-[3.5rem]">
              Your <span className="bg-gradient-to-r from-[#d7e4cc] via-[#e8f0e3] to-[#f6a51b] bg-clip-text text-transparent">good things</span><br />are here.
            </h1>
          </div>
        ...
        <section className="flex items-center p-7 sm:p-10 md:p-14">
          <div className="w-full max-w-xl">
            <h2 className="mt-2 text-3xl font-semibold tracking-[-0.035em] text-[#193b2a] sm:text-5xl">Sign in to your account</h2>
            <form onSubmit={submitHandler} className="mt-9 space-y-6">
              ...
              <label className="block text-sm text-[#55705d]">Password
                <span className="relative mt-2 flex items-center"><Lock size={17} className="absolute left-4 text-[#77907d]" /><input type={showPassword ? "text" : "password"} name="password" onChange={handleChange} value={formData.password} placeholder="Enter your password" required className="h-14 w-full border border-[#cbd8c9] bg-[#f7f8f2] pl-11 pr-10 text-sm text-[#193b2a] outline-none placeholder:text-[#9aaa9d] focus:border-secondary" /><button type="button" onClick={() => setShowPassword((value) => !value)} className="absolute right-4 text-[#55705d]" aria-label={showPassword ? "Hide password" : "Show password"}>{showPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button></span>
              </label>
              <button type="submit" className="h-14 w-full bg-secondary text-sm font-semibold text-white transition-colors hover:bg-[#193b2a]">Sign in</button>
            </form>

            <p className="mt-8 border-t border-[#d8e0d4] pt-6 text-sm text-[#55705d]">New here? <Link to="/signup" className="font-semibold text-secondary underline decoration-primary underline-offset-4">Create an account →</Link></p>
          </div>
        </section>
      </div>
    </main>
